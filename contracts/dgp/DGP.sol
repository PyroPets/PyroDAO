...
contract DGP {
    ...
    address payable public immutable governanceAddress; // address of governance contract
    ...
    constructor() {
        governanceAddress = payable(
            address(new Governance(payable(address(this))))
        );
    }
    ...
    function addProposal(ProposalType proposalType, address proposalAddress)
        public
    {
        Governance contractInterface = Governance(governanceAddress);
    ...
    function validateProposedContract(
        ...
        if (proposalType == ProposalType.GASSCHEDULE) {
            ...
            if (limit[0] >= minBlockGasLimit && limit[0] <= maxBlockGasLimit)
                return true;
            ...
            if (collateral[0] > 0 && collateral[0] <= maxGovernanceCollateral)
                return true;
    ...
    function getSchedule() public view returns (uint32[39] memory) {
        ...
        for (uint256 i = 0; i < 39; i++) {
            if (
                schedule[i] == 0 ||
                schedule[i] < defaultGasSchedule[i] / 100 ||
                schedule[i] > defaultGasSchedule[i] * 1000
            ) {
    ...
    function getBlockSize() public view returns (uint32[1] memory) {
        ...
        if (size[0] < minBlockSize || size[0] > maxBlockSize)
    ...
    function getMinGasPrice() public view returns (uint32[1] memory) {
        ...
        if (price[0] < 1 || price[0] > maxMinGasPrice)
    ...
    function getTransactionFeeRates() public view returns (uint64[3] memory) {
        ...
        if (rates[0] <= 0 || rates[0] > maxMinRelayTxFee)
            rates[0] = defaultMinRelayTxFee;
        if (rates[1] <= 0 || rates[1] > maxIncrementalRelayFee)
            rates[1] = defaultIncrementalRelayFee;
        if (rates[2] <= 0 || rates[2] > maxDustRelayFee)
            rates[2] = defaultDustRelayFee;
        return rates;
    }
    ...
    function getGovernanceCollateral() public view returns (uint256[1] memory) {
        ...
        uint256[1] memory collateral = contractInterface
            .getGovernanceCollateral();
        return
            collateral[0] > 0 && collateral[0] <= maxGovernanceCollateral
                ? collateral
                : [defaultGovernanceCollateral];
    }
