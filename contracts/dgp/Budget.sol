...
contract Budget {
    ...
    address payable public immutable governanceAddress;
    ...
    function startProposal(
        ...
        require(
            ...
            "Budget: Budget listing fee is required"
        );
        // must have duration and requested amount
        require(
            requested > 0,
            "Budget: Requested amount cannot be less than 1"
        );
        require(
            duration > 0,
            "Budget: Requested duration cannot be less than 1"
        );
        require(
            duration < 13,
            "Budget: Requested duration cannot be greater than 12"
        );
        // create new proposal
        _currentProposalId++;
        _proposalCount++;
    ...
    function voteForProposal(uint8 proposalId, Vote vote) public {
        Governance governanceInterface = Governance(governanceAddress);
    ...
    function settleBudget() public payable {
        ...
        if (payable(address(this)).balance > 0) {
            payable(address(0x0)).transfer(payable(address(this)).balance);
        }
    }
